...
- Use one deterministic placement policy in `commonMain`: scan rows from `0` through `board.rows - 1` and columns from `0` through `board.columns - 1`, selecting the first cell not occupied by the snake.
...
- For the default `20 x 20` board, row-major placement selects `Cell(column = 0, row = 0)` initially. The exact position is deterministic and is part of the common-rule contract; it is not generated independently by a UI or platform entry point.
...
- A fresh session exposes exactly one deterministic food cell that is in bounds and absent from the three-segment snake.
...
1. **Extend the shared immutable session model**:
   ...
   - Keep the model free of Compose, Android, desktop, browser, clock, and random-generator dependencies.

2. **Use deterministic bounded placement**:
   - Implement one private common-rule helper that scans every board coordinate in row-major order and returns the first unoccupied cell.
...
6. **Verify the business transition in common tests**:
   ...
   - Test controller publication with the injected/manual clock and deterministic logical steps; do not use wall-clock sleeps.
...
3. `composeApp/src/commonMain/kotlin/com/example/snake/game/rules/GameRules.kt`: initialization, deterministic free-cell selection, direction handling, boundary handling, normal movement, and atomic food collection.
...
1. `GameRules.startNewGame` creates the initial `GameState` and obtains its food from the shared bounded-placement helper.
2. `GameRules.advance` calls `Board.contains`, `Snake.head`, `Snake.moveTo`, and `Snake.moveToAndGrow`; it selects replacement food before constructing the successful collection state.
...
1. **Domain boundary**: Owns food validity, deterministic placement, growth, score increments, step outcomes, and all state-transition invariants. It does not render, schedule, persist, or implement future collision consequences.
...
4. **Platform boundary**: Owns only launch, lifecycle, input plumbing, and capability reporting. Equivalent movement sequences must not use different food-placement or collection code.
...
`startNewGame` → deterministic active `GameState` with one free-cell food → input request optionally sets `pendingDirection` → clock calls `GameRules.advance` → boundary check → normal `moveTo` or food `moveToAndGrow` plus `score + 10` and replacement selection → one `StepTransition` publication → `GameScreen` renders the matching score, snake, and one food marker.
...
### Implement deterministic placement - `GameRules.firstUnoccupiedCell`

1. **Responsibility**: Select a valid target without random or unbounded retry behavior.
2. **Contract**: Add a private helper equivalent to `private fun firstUnoccupiedCell(board: Board, snake: Snake): Cell?`.
3. **Logic**:
   - Iterate `row` in `0 until board.rows` and `column` in `0 until board.columns`.
   - Construct `Cell(column = column, row = row)` for each coordinate.
   - Return the first candidate for which `candidate !in snake.segments`.
   - Return `null` after the finite scan if no candidate exists; never loop indefinitely and never return an out-of-bounds cell.
4. **Completion criteria**: The helper returns `(0, 0)` for the default initial arrangement, excludes every snake segment, is deterministic across repeated calls, and returns `null` for a completely occupied board.
...
2. **Contract**: Preserve `fun startNewGame(board: Board = Board(20, 20)): GameState` and its current centered three-segment arrangement, `RIGHT` direction, `null` pending direction, `ACTIVE` status, and score `0`.
3. **Logic**:
   - Build and validate the existing centered initial segments exactly as before.
   - Call `firstUnoccupiedCell(board, Snake(initialSegments))`.
...
4. **Completion criteria**: Repeated calls return equivalent fresh states; the default food is deterministic, in bounds, not one of `(10, 10)`, `(9, 10)`, or `(8, 10)`, and the existing board-size validation remains intact.
...
2. **Contract**: Preserve `fun advance(state: GameState): StepTransition` and existing `NOT_ACTIVE`, `MOVED`, and `BOUNDARY_BLOCKED` semantics; add `FOOD_COLLECTED` and `FOOD_COLLECTION_BLOCKED` to `StepOutcome`.
3. **Common logic**:
...
5. **Collection logic**:
   ...
   - Call `firstUnoccupiedCell(state.board, grownSnake)` using the grown occupancy, not the old snake.
...
2. **Required changes**:
   - Keep `val state: StateFlow<GameState>`, `startNewGame`, `requestDirection`, `advanceForTest`, `startClock`, and `close` signatures unless compilation requires only the new state field updates.
   - Keep `startNewGame` based on `GameRules.startNewGame()` so it receives a fresh score, snake, and food.
   ...
   - Let `advanceForTest()` return `FOOD_COLLECTED` or `FOOD_COLLECTION_BLOCKED` without translating either into a game-over status.
...
2. **Mandatory common rule tests**:
   - Fresh initialization creates one food cell, selects the deterministic row-major result, keeps it in bounds, and keeps it outside all three initial segments.
   ...
   - Repeated initialization and repeated replacement selection are deterministic, finite, and never choose an occupied or out-of-bounds cell.
3. **Controller tests**:
...
4. **Placement**: Use the finite row-major scan and current snake occupancy as the sole placement policy. Do not use `Math.random`, platform random sources, retry loops, timers, or UI coordinates.
5. **Result handling**:
...
8. **Testing**: Name tests after business behavior, keep rule invariants and deterministic scenarios in `commonTest`, arrange adjacent food through valid state construction rather than pixel or event simulation, and include both negative and near-capacity cases.
...
10. **Documentation**: Document only non-obvious public or boundary decisions, especially deterministic row-major placement and the non-terminal `FOOD_COLLECTION_BLOCKED` capacity guard, matching the existing comment frequency and style.
...
2. **Performance constraints**:
   - Placement must finish in at most `board.columns * board.rows` candidate checks and must never retry indefinitely.
...
5. **Business rule constraints**:
   ...
   - Initial and replacement targets use the deterministic first unoccupied row-major cell.
...
7. **Technical constraints**:
   ...
   - Do not add a random source, repository, service, persistence model, or separate platform game loop.
   - Keep all state transitions deterministic and testable without sleeping.
8. **Data constraints**:
